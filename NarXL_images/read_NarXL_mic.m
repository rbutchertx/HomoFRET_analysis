...
for i = 1:2
    for j = 1:3
        ...
        nl_ims{i+2}(:,:,j) = double(imread(['tiffs2/' strain '_00' num2str(i) '.tif'],'Index',j));
        yl_ims{i}(:,:,j) = double(imread(['tiffs1/' strain '_on_00' num2str(i) '.tif'],'Index',j));
        yl_ims{i+2}(:,:,j) = double(imread(['tiffs1/' strain '_on_00' num2str(i) '.tif'],'Index',j));
    end
end
...
mNG_nl = mNG_nl(mNG_nl_ar>100);
mCh_nl = mCh_nl(mNG_nl_ar>100);
mNG_yl = mNG_yl(mNG_yl_ar>100);
mCh_yl = mCh_yl(mNG_yl_ar>100);

mCh_nl = rmoutliers(mCh_nl,'percentiles',[10 90]);
mCh_yl = rmoutliers(mCh_yl,'percentiles',[10 90]); 
%% Plotting
% close all
